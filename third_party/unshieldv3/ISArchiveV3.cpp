...
ISArchiveV3::ISArchiveV3(const std::string& apath)
    : m_path(apath)
{
    ...
    if (file_size <= sizeof(Header)) {
        throw std::runtime_error("InstallShield archive is truncated");
    }
    fin.read(reinterpret_cast<char*>(&hdr), sizeof(Header));
    if (!fin || hdr.signature1 != 0x8C655D13 || hdr.signature2 != 0x02013a || hdr.toc_address >= file_size) {
        throw std::runtime_error("Invalid InstallShield V3 archive");
    }
...
std::string ISArchiveV3::File::path() const {
